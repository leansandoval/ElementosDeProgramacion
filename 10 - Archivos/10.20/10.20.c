...
void ProcesarArch (T_Geo [],int,int[],float[],int,int[],int);
...
int CargarArch (T_Geo V[],int ce)
{
    ...
    fclose(pf);
    for(i=0; i<j; i++)
        printf("\nPosicion %d: LOCALIDAD: %d\t\tPROVINCIA: %d\t\tZONA: %d", i,V[i].cloc, V[i].cprov, V[i].czg);
    return(j);
}
...
void ProcesarArch(T_Geo V[], int cant, int VC[], float VK[],int ce, int VP[], int ce2)
{
    FILE *pf,*ss;
    T_Enc aux;
    int pos;
